...
async function fetchServerStatus() {
  ...
  if (!els.online && !els.dot) return;

  function renderStatus(data) {
    if (els.online) {
      if (data.online === null) {
        els.online.textContent = "Checking...";
      } else {
        els.online.textContent = data.online ? "🟢 Online" : "🔴 Offline";
      }
    ...
    if (els.dot) {
      els.dot.classList.toggle("offline", data.online === false);
    }

    if (els.badgeText) {
      if (data.online === null) {
        els.badgeText.textContent = "Checking Server...";
      } else {
        els.badgeText.textContent = data.online
          ? "Server Online"
          : "Server Offline";
      }
    }
    ...
    if (els.players) {
      els.players.textContent = data.players;
    }

    if (els.ping) {
      els.ping.textContent = data.ping;
    }
    ...
    if (els.address) {
      els.address.textContent = JAVA_ADDRESS;
    }
  }
  ...
  try {
    const response = await fetch(
      `https://api.mcstatus.io/v2/status/java/${JAVA_ADDRESS}`
    );
    ...
    renderStatus({
      ...
      players: data.online
        ...
        : "0/0",
      ping: data.latency ? `${data.latency} ms` : "—",
      version:
